...
class AccountMove(models.Model):
    ...
    def _reverse_move_vals(self, default_values, cancel=True):
        ...
        move_vals['ncf'] = ''
        move_vals['ncf_type'] = self.env['ir.sequence'].search(
            [('code', '=', 'out_refund')])
...
#     ===REVERSAL fields===
    parent_move_id = fields.Many2one('account.move', string='Parent Invoice',
                                     help="The move affected by the reversal.", domain="[('type','not in', ['in_refund', 'out_refund'])]")
    mod_ncf = fields.Char(
        'Modified NCF', related='parent_move_id.ncf', readonly=True)
